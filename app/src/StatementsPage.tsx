...
import { EXPENSE_PAGE_STORAGE_KEY } from './ExpensePage'
import { MEMO_PAGE_STORAGE_KEY } from './MemoPage'
import { GREEN_BEAN_ORDER_STORAGE_KEY } from './GreenBeanOrderPage'
import { STAFF_PAYROLL_STORAGE_KEY } from './StaffPayrollPage.tsx'
import {
...
import StatementSummaryTable, {
  ...
  type StatementSummaryTableMode,
} from './StatementSummaryTable'
import {
  ...
  if (savedMonthlyDates) {
    try {
      ...
      if (parsed && typeof parsed === 'object') {
        ...
        monthlyDateOverrides = migrateStatementMonthlyOverrideKeys(next)
      }
    } catch {
...
const normalizeStatementPageDocument = (value: unknown): StatementPageDocument => {
  ...
  const migratedMonthlyDateOverrides = migrateStatementMonthlyOverrideKeys(monthlyDateOverrides)
  return {
    ...
    monthlyDateOverrides: migratedMonthlyDateOverrides,
...
/** 거래처/품목/규격 동일성 판정 키 — POS·집계·override 키와 동일한 규칙(공유 모듈) */
const normalizeName = statementNameKey
...
const getBackupPayload = (
  ...
  records,
  pricingRules,
  activePage: parseAppActivePage(window.localStorage.getItem(ACTIVE_PAGE_STORAGE_KEY)),
...
const readBackupFile = async (fileHandle: FileSystemFileHandle): Promise<AppBackupPayload | null> => {
  ...
  return {
    ...
    activePage: parseAppActivePage(parsed.activePage),
...
/** 앱 셸 헤더(히어로)에 노출할 거래명세 요약 — 페이지가 마운트된 동안 갱신된다. */
export type StatementsHeroSummary = {
  recordCount: number
  grandTotalLabel: string
  selectedYear: string
  summaryClientCount: number
  scopeYmLabel: string
  scopeMonthTotalLabel: string
  scopeMonthClientCount: number
  scopeMonthRecordCount: number
  saveState: DocumentSaveState
  lastSavedAt: string
}

type StatementsPageProps = {
  onHeroSummaryChange?: (summary: StatementsHeroSummary | null) => void
  onStickyHScrollVisibleChange?: (visible: boolean) => void
  onRestoreActivePage?: (page: AppActivePage) => void
}

export default function StatementsPage({
  onHeroSummaryChange,
  onStickyHScrollVisibleChange,
  onRestoreActivePage,
}: StatementsPageProps) {
      ...
      try {
        ...
        if (backupPayload) {
          ...
          if (backupPayload.activePage) {
            ...
            onRestoreActivePage?.(backupPayload.activePage)
          }
  ...
  const summaryRows = useMemo(() => {
    ...
    filteredRecords.forEach((record) => {
      const monthIndex = Number(record.deliveryDate.slice(5, 7)) - 1
      // 표기 변형(띄어쓰기 등)이 다른 같은 거래처가 별도 행으로 갈라지지 않게 정규화 키로 묶는다
      const clientKey = normalizeName(record.clientName)
      const existing =
        grouped.get(clientKey) ??
        ({
          clientName: record.clientName.trim() || record.clientName,
          totalAmount: 0,
          share: 0,
  ...
  useEffect(() => {
    if (summaryTableMode === 'amounts') {
      setSummaryEditingDateKey(null)
    }
  }, [summaryTableMode])
  ...
  }, [activeView, records, filteredRecords, summaryRows, selectedYear])

  useEffect(() => {
    onStickyHScrollVisibleChange?.(statementStickyHScrollVisible)
  }, [onStickyHScrollVisibleChange, statementStickyHScrollVisible])

  useEffect(() => {
    onHeroSummaryChange?.({
      recordCount: records.length,
      grandTotalLabel: `${formatCurrency(grandTotal)}원`,
      selectedYear,
      summaryClientCount: summaryRows.length,
      scopeYmLabel: statementScopeYmLabel,
      scopeMonthTotalLabel: `${formatCurrency(statementScopeMonthSummary.totalAmount)}원`,
      scopeMonthClientCount: statementScopeMonthSummary.distinctClientCards,
      scopeMonthRecordCount: statementScopeMonthSummary.recordCount,
      saveState: statementSaveState,
      lastSavedAt: statementLastSavedAt,
    })
  }, [
    ...
    selectedYear,
    summaryRows.length,
    statementScopeYmLabel,
    statementScopeMonthSummary,
    statementSaveState,
    statementLastSavedAt,
  ])

  /* 페이지를 떠나면 셸 헤더 요약·하단 스크롤바 패딩을 초기화 */
  useEffect(
    () => () => {
      onHeroSummaryChange?.(null)
      onStickyHScrollVisibleChange?.(false)
    },
    [onHeroSummaryChange, onStickyHScrollVisibleChange],
  )
  ...
  const handleDelete = (id: string) => {
    ...
    const summary = target
      ? `${formatDateLabel(target.deliveryDate)} · ${target.clientName || '미지정'} · ${target.itemName} (${formatCurrency(target.totalAmount)}원)`
      : '선택한 거래명세'
    if (!window.confirm(`${summary} 항목을 삭제할까요?\n삭제하면 되돌릴 수 없습니다.`)) {
      return
    }
    ...
    <>
      ...
      <main className="statements-main">
        <section className="panel statements-records-panel">
          ...
          {activeView === 'summary' ? (
            <div className="statements-summary-toolbar no-print">
              ...
              <label className="statements-summary-hide-empty">
                <input
                  type="checkbox"
                  checked={summaryHideEmptyClients}
                  onChange={(event) => setSummaryHideEmptyClients(event.target.checked)}
                />
                선택 월 납품 없는 거래처 숨기
              </label>
          ...
          {activeView === 'records' ? (
            ...
            <div
              className={[
                'table-wrapper',
                'statements-main-table-hscroll',
                !showAllSummaryMonths ? 'statements-main-table-hscroll-single-month' : '',
                summaryTableMode === 'amounts' ? 'statements-main-table-hscroll-amounts' : '',
              ]
                .filter(Boolean)
                .join(' ')}
            ...
            >
              <StatementSummaryTable
                ...
                editingDateKey={summaryEditingDateKey}
                onEditingDateKeyChange={setSummaryEditingDateKey}
                onMonthDateChange={handleSummaryMonthDateChange}
              />
            </div>
          )}
          </div>
              ...
              <div className="statement-client-hub-panel statement-client-hub-panel-summary">
                <div className="statement-client-hub-panel-toolbar">
                  ...
                  <span className="statement-client-hub-hint">
                    발행·입금일자는 메인 「월별 납품현황」과 동일하게 저장됩니다.
                  </span>
                </div>
